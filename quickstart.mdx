...
This example creates a **standard** mode escrow (default). For product sales without a contract, use `mode: "quick"`. Optionally set **`completionDeadlineDays`** (integer **3–14**, default **7**) for how long the buyer has after deliver before auto-release. For merchant checkout (credits, top-ups, public pay links), use [POST /charges](/api-reference/charges/create-charge) — do not send `mode: "instant"`. See [Escrow Modes](/api-reference/escrows/overview#escrow-modes).
