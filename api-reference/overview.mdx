...
- **[Overview](/api-reference/escrows/overview)** — Introduction to escrows, [modes](/api-reference/escrows/overview#escrow-modes) (standard, quick), auto-release after deliver (`completionDeadlineDays` 3–14), [Quick Escrow with Proof](/guides/quick-escrow-with-proof), statuses, and lifecycle
...
A [charge](/api-reference/charges/overview) is a public payment link for **your** business (credits, top-ups, one-off sales). There is no `buyerEmail`: anyone with the URL can pay, and payment completes and settles immediately.

- **[Overview](/api-reference/charges/overview)** — Charge vs escrow, `merchantReference` vs `receiptEmail`, idempotency
- `POST /charges` - [Create a charge](/api-reference/charges/create-charge) — returns a public checkout URL
- `GET /charges/:id` - [Get a charge](/api-reference/charges/get-charge) — by DHMAD id or your `merchantReference`
...
|------|----------|----------|------------|
| `standard` | Services, freelance work | Yes | Buyer confirms after delivery (or auto-release after `completionDeadlineDays`) |
| `quick` | Products between two users | No | Buyer confirms after delivery (or auto-release after `completionDeadlineDays`) |

`mode: "instant"` is rejected. Merchant checkout (credits, top-ups, public pay links) is **[POST /charges](/api-reference/charges/overview)**.
