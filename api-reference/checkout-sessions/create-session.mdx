...
<ParamField body="action" type="string" required>
  ...
  **By escrow mode:**
  ...
  - **quick** — no contract; `sign_contract` returns 400 (`QUICK_ESCROW_NO_CONTRACT`). Typical flow: `accept_pay`, then buyer `complete`. Seller may call [Deliver Escrow](/api-reference/escrows/deliver-escrow) first, but the buyer can `complete` directly from `paid` without deliver. With [Quick Escrow with Proof](/guides/quick-escrow-with-proof), use `submit_proof` (seller) and `review_proof` (buyer) while `proofInfo.phase` requires them; early release from `paid` is blocked until proof is accepted. `add_funds` / `refund_funds` apply when `paid` or `delivered`.
