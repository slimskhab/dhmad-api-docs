...
<ParamField body="action" type="string" required>
  ...
  **Cancellation actions (paid or delivered escrows):**
  ...
  - **Sellers** do not use `request_cancellation`. They cancel directly on dhmad.tn (immediate refund when allowed). Targeting the seller for `request_cancellation` returns **400** (`CANCELLATION_REQUEST_BUYER_ONLY` or `CHECKOUT_TARGET_NOT_BUYER`).
...
| `CANCELLATION_REQUEST_BUYER_ONLY` | Action is `request_cancellation` but `targetUserEmail` is the seller. Sellers cancel directly on dhmad.tn. |
