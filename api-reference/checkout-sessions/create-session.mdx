...
<ParamField body="action" type="string" required>
  ...
  **Funds adjustment actions (paid or delivered escrows):**
  - `add_funds` — **Buyer only.** Target the buyer's email. The buyer adds more money from their DHMAD wallet into the escrow. Fees apply to the **added** amount only. Requires action step-up on checkout. Not supported for escrows paid via guest checkout.
  - `refund_funds` — **Seller only.** Target the seller's email. The seller refunds part or all of the remaining ledger balance to the buyer's wallet. Refunding the **full** remaining balance cancels the escrow. Requires action step-up on checkout. Not supported for guest-checkout escrows.
  - Targeting the wrong party returns **400** (`CHECKOUT_TARGET_NOT_BUYER` / `CHECKOUT_TARGET_NOT_SELLER`).

  **Progressive release actions ([hourly contracts](/guides/progressive-release), paid escrows):**
  - `approve_release` — **Client only.** Target the buyer's email and pass the payout request in `metadata.requestId`. Releases that period to the seller immediately. Requires action step-up on checkout.
  - `reject_release` — **Client only.** Same targeting and `metadata.requestId`. Opens a dispute scoped to that period; the amount stays reserved and later periods keep running. No step-up (no money moves).
  ...
  **By escrow mode:**
  - **standard** — all actions apply when status allows (except proof actions). `complete` works from `paid` or `delivered` unless `completionPolicy` is set (then deliver is required). `add_funds` / `refund_funds` apply when `paid` or `delivered`.
  - **quick** — no contract; `sign_contract` returns 400 (`QUICK_ESCROW_NO_CONTRACT`). Typical flow: `accept_pay`, then buyer `complete`. Seller may call [Deliver Escrow](/api-reference/escrows/deliver-escrow) first; without `completionPolicy` the buyer can `complete` from `paid`. With [delivery confirmation](/guides/completion-policy), `complete` from `paid` returns `DELIVERY_REQUIRED`. With [Quick Escrow with Proof](/guides/quick-escrow-with-proof), use `submit_proof` (seller) and `review_proof` (buyer) while `proofInfo.phase` requires them; early release from `paid` is blocked until proof is accepted. `add_funds` / `refund_funds` apply when `paid` or `delivered`.
  - A [charge](/api-reference/charges/overview) id returns **409** `CHARGE_NOT_AN_ESCROW`. Use the checkout URL from [Create Charge](/api-reference/charges/create-charge).

  **`complete` action:** Buyer releases funds. Valid when escrow status is `paid` or `delivered`, except when [`completionPolicy`](/guides/completion-policy) is set — then deliver is required first (`400 DELIVERY_REQUIRED`). Without that policy the buyer can release early from `paid`. Returns **409** (`PROOF_NOT_ACCEPTED`) for Quick Escrow with Proof when proof is not yet accepted.
</ParamField>
...
| `DELIVERY_REQUIRED` | Action is `complete` but `completionPolicy` requires the seller to deliver first. |
| `DISPUTE_ALREADY_OPENED` | Action is `dispute` but a dispute has already been opened. |
| `DISPUTE_WINDOW_CLOSED` | Action is `dispute` but `completionDeadline` has passed on a delivery-confirmation escrow. |
...
| `CHECKOUT_TARGET_NOT_BUYER` | Action is `request_cancellation`, `add_funds`, or another buyer-only action but `targetUserEmail` is not the buyer. |
| `CHECKOUT_TARGET_NOT_SELLER` | Action is `cancel_escrow`, `refund_funds`, or another seller-only action but `targetUserEmail` is not the seller. |
...
| `FUNDS_ADJUSTMENT_INVALID_STATUS` | Action is `add_funds` or `refund_funds` but escrow is not `paid` or `delivered`. |
| `GUEST_ESCROW_ADJUSTMENTS_NOT_SUPPORTED` | Action is `add_funds` or `refund_funds` but the escrow was paid via guest checkout. |
| `BUYER_NOT_LINKED` | Action is a funds adjustment but the buyer account is not linked yet. |
| `DISPUTE_OPEN` | Action is `add_funds` or `refund_funds` but a dispute is open. |
| `CANCELLATION_PENDING` | Action is `add_funds` or `refund_funds` but a cancellation request is pending. |
| `PROGRESSIVE_RELEASE_NOT_ENABLED` | Action is `approve_release` or `reject_release` but the escrow does not use progressive release. |
| `RELEASE_REVIEW_INVALID_STATUS` | Action is `approve_release` or `reject_release` but the escrow is not `paid`. |
| `RELEASE_REQUEST_ID_REQUIRED` | Action is `approve_release` or `reject_release` but `metadata.requestId` is missing. |
| `RELEASE_REQUEST_NOT_FOUND` | No payout request with that `requestId` exists on the escrow. |
| `RELEASE_REQUEST_NOT_PENDING` | The named payout request has already been decided. |
...
A [charge](/api-reference/charges/overview) id returns **409**:

```json
{
  "error": "Conflict",
  "code": "CHARGE_NOT_AN_ESCROW",
  "message": "This id is a charge. Use the /v1/charges endpoints."
}
```
...
<Info title="Charges">
  Merchant checkout uses [POST /charges](/api-reference/charges/create-charge), which already returns a public checkout URL. Creating a session on a charge id returns **409** `CHARGE_NOT_AN_ESCROW`.
</Info>
