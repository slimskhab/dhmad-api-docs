...
<ParamField body="action" type="string" required>
  ...
  **By escrow mode:**
  - **standard** — all actions apply when status allows.
  - **quick** — no contract; `sign_contract` returns 400 (`QUICK_ESCROW_NO_CONTRACT`). Use `accept_pay` then `complete`.
  - **instant** — only `accept_pay` (and dispute/cancellation actions) are valid; `sign_contract` and `complete` return 400.
</ParamField>
