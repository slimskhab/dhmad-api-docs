...
<ParamField body="action" type="string" required>
  ...
  **Progressive release actions ([hourly contracts](/guides/progressive-release), paid escrows):**
  ...
  - Both require `progressiveRelease.enabled` on the escrow and a `pending` payout request. The seller never acts on these — payout requests are created by your backend, optionally on the seller's behalf.
