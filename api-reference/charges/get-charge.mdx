...
Use this to reconcile when a webhook was missed: if `status` is `completed`, credit your user exactly as you would from **`charge.status.updated`**.

[Get Escrow](/api-reference/escrows/get-escrow) with this id returns **404**. Escrow lifecycle endpoints return **409** `CHARGE_NOT_AN_ESCROW`.
...
- Credit from `merchantReference` when `status` is `completed`. For cash-on-delivery, `completed` can later move to `disputed` then `cancelled` — the same `charge.status.updated` event fires again.
