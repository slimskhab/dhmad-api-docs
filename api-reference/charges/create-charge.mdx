...
Payment completes the charge and settles the funds into your business wallet in one step — there is no deliver call and no `completionDeadlineDays`. Subscribe to **`charge.status.updated`** (see [Webhooks](/guides/webhooks#chargestatusupdated)).

Do not pass this id to `/v1/escrows/:id` — GET returns **404**; deliver, sessions, and other escrow actions return **409** `CHARGE_NOT_AN_ESCROW`.
