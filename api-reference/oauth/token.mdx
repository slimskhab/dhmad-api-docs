...
Exchange an authorization code for an access token (and optionally an ID token if `openid` scope was requested). This must be done server-side — never expose your client secret in client-side code.
