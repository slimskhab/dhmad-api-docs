...
<ParamField response="escrow.mode" type="string">
  Escrow mode: `standard` (contract + sign + pay + complete; deliver optional), `quick` (no contract; pay + buyer `complete`; deliver optional), or `instant` (no contract; auto-complete on deliver). See [Escrow Modes](/api-reference/escrows/overview#escrow-modes).
</ParamField>
