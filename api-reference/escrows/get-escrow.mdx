...
<ParamField response="escrow.amount" type="number">
  Transaction amount (face amount in `currency`). After `add_funds` / `refund_funds`
  adjustments this reflects the current commercial amount.
</ParamField>
...
<ParamField response="escrow.availableForReleaseTnd" type="number">
  ...
  (`amountTnd` − `reservedTnd`). Zero means the contract needs a top-up.
</ParamField>
...
<ParamField response="escrow.releaseRequests" type="array">
  ...
  (`pending`, `approved`, `disputed`, `released`, `voided`), `approvalDeadline`,
  `sellerNetTnd`, and any admin `resolution`. See
