...
- Checkout actions: `sign_contract` → `accept_pay` → `complete`. Seller may call [Deliver Escrow](/api-reference/escrows/deliver-escrow) first, but it is **optional** — the buyer can `complete` directly from `paid` (early release) if they are already satisfied.
...
- Checkout actions: `accept_pay` → `complete`. Seller may call [Deliver Escrow](/api-reference/escrows/deliver-escrow) first, but it is **optional** — the buyer can `complete` directly from `paid` without waiting for deliver. The `sign_contract` action returns **400** (`QUICK_ESCROW_NO_CONTRACT`).
...
- Progressive-release creates reject the field (`400`). [Charges](/api-reference/charges/overview) never enter `delivered` and do not use this window.
...
| Buyer confirms after delivery | Yes (or early release from `paid`) | Yes (or early release from `paid`) |
| `contractTerms` | Required | Not allowed |
| `feeBearer` | Optional (default `seller`) | Optional (default `seller`) |
| `completionDeadlineDays` | Optional (3–14, default 7) | Optional (3–14, default 7) |
...
- **Complete** — Buyer releases funds. Valid when escrow is `paid` or `delivered`; deliver is not required first. Not used on [charges](/api-reference/charges/overview).
