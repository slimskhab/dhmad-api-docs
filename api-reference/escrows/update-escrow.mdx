...
A [charge](/api-reference/charges/overview) id returns **409** `CHARGE_NOT_AN_ESCROW`.
...
<ParamField body="feeBearer" type="string" required={false}>
  Who pays platform + developer fees: `"seller"` or `"buyer"`. Changing this (or amount/currency) clears existing contract signatures.
</ParamField>
...
<ParamField response="escrow" type="object">
  Updated escrow object with all current fields (including `feeBearer` and `buyerFundingTnd`)
</ParamField>
