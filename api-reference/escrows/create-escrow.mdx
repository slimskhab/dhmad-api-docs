...
<ParamField body="progressiveRelease" type="object" required={false}>
  **Hourly / recurring contracts** — allowed for `standard` and `quick` modes (returns **400** for `instant`). Lets one funded escrow pay out in slices, one per billing period. See the [Progressive Release guide](/guides/progressive-release).
