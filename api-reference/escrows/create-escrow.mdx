...
Use **standard** mode (default) for full contract/sign/deliver/complete flow. Use **instant** mode for digital goods (e.g. in-app coins, AI credits): no contract is created, the buyer only confirms and pays, and when you call deliver the escrow auto-completes and funds are released immediately. See [Instant escrow mode](/api-reference/escrows/overview#instant-escrow-mode) in the Escrows overview.
...
<ParamField body="estimatedDeliveryDays" type="number" required={false}>
  Estimated delivery days (must be at least 1). **Required** when `mode` is `standard`. Optional when `mode` is `instant`; defaults to `1` if omitted.
</ParamField>

<ParamField body="contractTerms" type="string" required={false}>
  Contract terms describing the transaction agreement. **Required** when `mode` is `standard`; a contract is created with these terms. Omit when `mode` is `instant` (no contract is used).
</ParamField>

<ParamField body="autoSign" type="boolean" required={false}>
  If `true`, the contract will be automatically signed by the seller using the signature configured in the developer dashboard. Requires a signature to be set up first. **Note:** Auto-sign is only available when `sellerEmail` is not provided (i.e., when using your associated user account as the seller). When `sellerEmail` is provided, the seller must sign the contract themselves. Not applicable when `mode` is `instant` (returns 400 if set). Defaults to `false`.
</ParamField>

<ParamField body="mode" type="string" required={false}>
  Escrow mode: `standard` (default) or `instant`. Use `instant` for digital goods (e.g. in-app coins, AI credits) to skip the contract and get a one-step buyer flow (pay only); calling deliver then auto-completes and releases funds. Defaults to `standard`.
</ParamField>
...
<ParamField response="escrow.mode" type="string">
  Escrow mode: `standard` or `instant`. Present when created via API; defaults to `standard` for existing escrows.
</ParamField>
...
<ParamField response="contract" type="object|null">
  Contract object when `mode` is `standard`; `null` when `mode` is `instant` (no contract is created).
</ParamField>

<ParamField response="contract.id" type="string">
  Unique identifier for the contract (only when `contract` is not null)
</ParamField>

<ParamField response="contract.pdfUrl" type="string|null">
  URL to the generated contract PDF, or `null` if PDF generation is pending
</ParamField>
...
**Auto-Sign With Instant Mode**

```json
{
  "error": "Bad Request",
  "message": "Auto-sign is not applicable for instant mode (no contract is created)"
}
```
...
**Daily Escrow Limit Exceeded**

```json
{
  "error": "Rate Limit Exceeded",
  "message": "You have reached your daily limit of 10 escrows. You can create more escrows tomorrow.",
  "limit": 10,
  "count": 10,
  "remaining": 0
}
...
<Note>
  To use the `autoSign` feature, you must first set up your signature in the developer dashboard. Use the `PUT /developer/signature` endpoint to upload your signature image (base64 encoded). Once configured, you can enable `autoSign: true` when creating **standard** mode escrows to automatically sign contracts as the seller. `autoSign` is not applicable for `mode: "instant"`.
</Note>

<Info title="Instant mode example">
  For digital goods (e.g. 100 AI coins), create with `mode: "instant"` and omit `contractTerms` and optionally `estimatedDeliveryDays` (defaults to 1). The response will include `escrow.mode: "instant"` and `contract: null`. Create a checkout session with `action: "accept_pay"` only; the buyer pays in one step. When you deliver via `POST /api/v1/escrows/:id/deliver`, the escrow auto-completes and funds are released. See [Instant escrow mode](/api-reference/escrows/overview#instant-escrow-mode).
</Info>
