...
- **`quick`** — no contract; pay → buyer `complete` to release funds. Seller deliver is optional. Use with `sellerEmail` for product sales without a contract. Optionally add **`fulfillmentPolicy`** for [Quick Escrow with Proof](/guides/quick-escrow-with-proof) (seller uploads proof before delivery unlocks).
...
<ParamField body="completionDeadlineDays" type="integer" required={false}>
  Days the buyer has after deliver to release funds, reject delivery, or open a dispute. Integer **3–14** (platform default **7** when omitted). Frozen on the escrow at creation. After deliver, silence auto-completes the escrow and releases funds. Not allowed on progressive-release creates (**400**). [Charges](/api-reference/charges/overview) do not use this field.
</ParamField>
