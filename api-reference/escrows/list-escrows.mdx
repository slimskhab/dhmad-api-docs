...
[Charges](/api-reference/charges/overview) are excluded. Look those up with [Get Charge](/api-reference/charges/get-charge).
...
<ParamField response="escrows[].mode" type="string">
  Escrow mode: `standard` or `quick`. See [Escrow Modes](/api-reference/escrows/overview#escrow-modes).
</ParamField>
...
<Info>
  If no filters are provided, the endpoint returns all escrows where you are either the seller or buyer, sorted by creation date (newest first). [Charges](/api-reference/charges/overview) are not included.
</Info>
