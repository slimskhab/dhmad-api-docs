...
Sent on [progressive-release](/guides/progressive-release) escrows, which pay out one slice per billing period. Every payload carries `data.releaseRequest` plus the escrow's live reservation figures, so you can reconcile without a follow-up request.
...
<ParamField response="data.releaseRequest" type="object">
  ...
  optional `reference`, `status`, `amountTnd`, and — depending on the event —
...
<ParamField response="data.escrow.availableForReleaseTnd" type="number">
  Balance a new payout request may claim. Zero means a top-up is needed.
</ParamField>
