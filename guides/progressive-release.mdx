...
<Info>
  Progressive release composes with `mode: "quick"` (no contract) or `mode: "standard"` (with a signed contract). It is not available on [charges](/api-reference/charges/overview), which complete on payment. Do not send `completionDeadlineDays` or [`completionPolicy`](/guides/completion-policy) on a progressive-release create (**400**).
</Info>
