...
- **Developer portal**: [developer.sandbox.dhmad.tn](https://developer.sandbox.dhmad.tn) — create and manage **sandbox API keys** (`sk_sandbox_...`)
...
| ----------- | ------------- | ------------------------------------- |
| **Live**    | `sk_live_...` | `https://api.dhmad.tn/api` (production)   |
...
1. Open the **sandbox** developer portal: [developer.sandbox.dhmad.tn](https://developer.sandbox.dhmad.tn)
2. Sign up or log in (sandbox has its own user/developer data)
3. [Associate your developer account](/docs/developer-portal/associating-accounts) with a user account (use a test user on sandbox.dhmad.tn)
4. Go to **API Keys** and click **Create API Key**
5. Keys created in the sandbox developer portal are **sandbox keys** (`sk_sandbox_...`). Use them only with `https://sandbox.dhmad.tn/api`.

<Info>
  Sandbox and production have **separate accounts and data**. You need a user account and developer account on **sandbox.dhmad.tn** / **developer.sandbox.dhmad.tn** to create and use sandbox API keys.
</Info>
...
2. Change your app’s API base URL from `https://sandbox.dhmad.tn/api` to `https://api.dhmad.tn/api`
3. Replace `sk_sandbox_...` with `sk_live_...` in your config/secrets
4. Ensure your production developer account is [associated](/docs/developer-portal/associating-accounts) with the correct production user account
...
| **API base**     | `https://sandbox.dhmad.tn/api`                                          | `https://api.dhmad.tn/api`                                       |
| **Developer portal** | [developer.sandbox.dhmad.tn](https://developer.sandbox.dhmad.tn)  | [developer.dhmad.tn](https://developer.dhmad.tn)                 |
| **API key prefix**  | `sk_sandbox_`                                                       | `sk_live_`                                                       |
| **Data**         | Test only; separate DB                                                  | Live data                                                        |

## Next Steps

- **[Getting Started](/docs/getting-started)** — Create a developer account and get API keys
- **[Authentication](/docs/authentication)** — Understand API key usage and headers
- **[Quickstart](/docs/quickstart)** — Build a complete integration example
- **[Create Escrow](/docs/api-reference/escrows/create-escrow)** — Create your first escrow in the sandbox
