...
After `accepted` or `accepted_by_timeout`, the buyer may release funds with a `complete` checkout session from `paid`, or the seller may call deliver first (optional unless you also send [`completionPolicy`](/guides/completion-policy)) and the buyer completes from `delivered`. The seller's deliver call returns **409** (`PROOF_NOT_ACCEPTED`) until proof is accepted. Stack delivery confirmation when the buyer must wait for deliver after proof.
