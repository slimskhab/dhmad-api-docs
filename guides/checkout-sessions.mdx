...
For escrows created with `mode: "quick"` (e.g. physical or digital products between two users), use **accept_pay** then **complete** after the seller delivers. There is no contract to sign — `sign_contract` returns 400. See [Escrow Modes — Quick](/api-reference/escrows/overview#quick-mode) in the Escrows overview.
...
2. **`accept_cancellation`** or **`reject_cancellation`** — After the buyer requests, target the **seller's** email so they can accept (refund) or reject.

**Sellers** do not use `request_cancellation`. They cancel directly on dhmad.tn (immediate refund when allowed). Creating a `request_cancellation` session with the seller's email returns **400** (`CANCELLATION_REQUEST_BUYER_ONLY`).
