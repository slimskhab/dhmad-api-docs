...
Yes, for **standard** and **quick** escrows. After payment (`paid`), the buyer can use a `complete` checkout session to release funds without the seller calling [Deliver Escrow](/api-reference/escrows/deliver-escrow). Deliver is supplementary — it records that the seller marked delivery, but it is not a prerequisite for `complete`.

**Exception:** [Quick Escrow with Proof](/guides/quick-escrow-with-proof) blocks early release from `paid` until the buyer accepts the seller's purchase proof (`409 PROOF_NOT_ACCEPTED`).
