...
- **action** — The action type (e.g., `sign_contract`, `accept_pay`, `submit_proof`, `review_proof`, `complete`, `add_funds`, `refund_funds`)
...
The checkout page shows a summary of the escrow: amount, **platform service fee** (DHMAD's fee), and, if the escrow was created with a developer fee, the **application / app fee** (with your app name when configured). Which party pays those fees follows `feeBearer` (`"seller"` default or `"buyer"`). Use `buyerFundingTnd` for the amount the buyer actually pays.

#### Guest payment (no DHMAD login)

On a public checkout URL — including [charges](/api-reference/charges/overview) and guest `accept_pay` — the payer can pay **without** creating an account. They pick a payment method (e.g. bank transfer, D17, Flouci, PayPal), complete the payment using the instructions shown, and upload a proof file when the method requires it. An **admin must approve** the proof; only then does the payment move to `paid`. For charges, subscribe to **`charge.status.updated`**. For escrows, rely on **`escrow.status.updated`**.
...
- **Seller cancellation before approval:** pending proofs are closed automatically; you may receive **`escrow.deposit_proof.rejected`** (with a system reason) and a status webhook when the payment becomes `cancelled`. The checkout session may be marked `cancelled`. Treat **webhooks** as the source of truth, not only the browser redirect.

#### Standard flow (logged-in buyer)
...
### What about charges (credits, top-ups, public pay links)?

Use **[POST /charges](/api-reference/charges/create-charge)**. That call already returns a public checkout URL — do not create an escrow checkout session on a charge id (**409** `CHARGE_NOT_AN_ESCROW`). Payment completes and settles in one step. Subscribe to **`charge.status.updated`**. See [Charges](/api-reference/charges/overview).

### Can the buyer release funds without the seller delivering?

Yes, for **standard** and **quick** escrows **unless** you set [`completionPolicy`](/guides/completion-policy). After payment (`paid`), the buyer can use a `complete` checkout session to release funds without the seller calling [Deliver Escrow](/api-reference/escrows/deliver-escrow). Deliver is supplementary — it records that the seller marked delivery, but it is not a prerequisite for `complete`.

**Exceptions:**
- [Delivery confirmation](/guides/completion-policy) — `complete` from `paid` returns `DELIVERY_REQUIRED`. Deliver first.
- [Quick Escrow with Proof](/guides/quick-escrow-with-proof) — early release from `paid` is blocked until the buyer accepts the seller's purchase proof (`409 PROOF_NOT_ACCEPTED`).

### What about quick escrows (products without a contract)?

For escrows created with `mode: "quick"` (e.g. physical or digital products between two users), use **accept_pay** then **complete** to release funds. The seller may call [Deliver Escrow](/api-reference/escrows/deliver-escrow) first, but deliver is **optional** — if the buyer is already satisfied after payment, send them a `complete` checkout session while the escrow is still `paid`. There is no contract to sign — `sign_contract` returns 400. See [Escrow Modes — Quick](/api-reference/escrows/overview#quick-mode) in the Escrows overview.
...
### How do add funds and refund funds checkout sessions work?

While an escrow is **`paid` or `delivered`** (and was **not** funded via guest checkout), parties can adjust the held amount via hosted checkout:

1. **`add_funds`** — Target the **buyer's** email. The buyer adds more money from their DHMAD wallet into the escrow. Fees apply to the **added** amount only. DHMAD requires action step-up (email OTP) on the checkout page.
2. **`refund_funds`** — Target the **seller's** email. The seller refunds part or all of the remaining ledger balance to the buyer's wallet. Refunding the **full** remaining balance cancels the escrow. Also requires action step-up.

**Do not mix actions:** targeting the seller for `add_funds` or the buyer for `refund_funds` returns **400**.

Subscribe to **`escrow.funds.added`** and **`escrow.funds.refunded`** webhooks to update your backend. A full refund that cancels the escrow also emits `escrow.status.updated` (to `cancelled`); the refund webhook payload includes `adjustment.cancelled: true` in that case.

Guest-checkout escrows and open disputes / pending cancellation requests cannot use these actions (`GUEST_ESCROW_ADJUSTMENTS_NOT_SUPPORTED`, `DISPUTE_OPEN`, `CANCELLATION_PENDING`).
